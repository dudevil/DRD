...
train_y = theano.shared(train_y.astype(np.int32))
...
valid_y = theano.shared(valid_y.astype(np.int32))
...
pred = T.argmax(
    output.get_output(X_batch, deterministic=True), axis=1)
...
for x_next, y_next in dloader.train_gen():
    ...
    train_y.set_value(y_next.astype(np.int32), borrow=True)