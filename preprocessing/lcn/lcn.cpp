...
cv::Mat lcn_color(cv::Mat const& img, int const ksize, bool const normalize_global)
{
	...
	return apply_lcn(img, ksize, normalize_global);
}
...
cv::Mat lcn_yuv(cv::Mat const& img, int const ksize, bool const normalize_global)
{
	...
	img_yuv.convertTo(img_yuv, CV_32FC3, 1.0 / 255.0);
	...
	// LCN on Y channel
	channels[0] = normalize_local_contrast(channels[0], ksize);
	...
	channels[0].convertTo(channels[0], channels[0].type(), 1 / (2 * gaussian_99_percentile));
...
int main(int const argc, char const* const argv[])
{
	...
	// read command line
	std::string const input_filename  = cli.get<std::string>("int");
